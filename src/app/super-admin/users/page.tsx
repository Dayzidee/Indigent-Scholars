...
export default function SuperAdminUsers() {
  ...
  const getRoleBadge = (role: string) => {
    switch(role) {
      ...
      case 'super_admin': return <span className="px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest bg-amber-500/10 text-amber-400 border border-amber-500/20">Super Admin</span>;
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <nav className="flex items-center gap-2 text-xs font-medium text-slate-500 mb-6">
        ...
        <span className="text-amber-400">User CRM</span>
      </nav>

      <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
          <h1 className="text-3xl font-black text-white mb-2 tracking-tight">Platform Registry</h1>
        ...
        <div className="flex gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-12 pr-4 py-3 bg-slate-900 border border-slate-800 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500/50 w-80 transition-all font-medium"
            />
          </div>

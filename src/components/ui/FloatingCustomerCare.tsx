    ...
    <div className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-[100] flex flex-col items-end gap-3">
      <AnimatePresence>
        {isHovered && !isOpen && (
          <motion.div
            ...
            className="hidden md:block bg-primary text-white px-4 py-2 rounded-xl text-xs font-bold shadow-xl border border-white/10"
          >
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <Card className="shadow-2xl border border-zinc-100 overflow-hidden rounded-3xl bg-white flex flex-col h-[400px] md:h-[500px] max-h-[70vh]">
              {/* Header */}
              <div className="bg-primary scholar-gradient p-4 md:p-6 text-white flex justify-between items-center group">
                <div className="flex items-center gap-3 md:gap-4">
                   <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/20 flex items-center justify-center border border-white/20 shrink-0">
              ...
              {/* Messages Area */}
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 bg-zinc-50/50 scrollbar-hide">
                {messages.map((msg) => (
                  <div key={msg.id} className={cn("flex flex-col", msg.sender === 'user' ? "items-end" : "items-start")}>
                    <div className={cn(
                      ...
                      msg.sender === 'user' ? "bg-primary text-white rounded-tr-none" : "bg-white text-on-surface rounded-tl-none border border-zinc-100"
                    )}>
                      {msg.text}
                    </div>
              ...
              <div className="p-3 md:p-4 bg-white border-t border-zinc-100">
                <div className="relative flex items-center bg-zinc-50 rounded-2xl border border-zinc-200 focus-within:border-primary/30 focus-within:ring-2 focus-within:ring-primary/10 transition-all">
                  <input 
                    type="text" 
                    value={inputValue}
